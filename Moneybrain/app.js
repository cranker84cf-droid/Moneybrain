...
function importStatementTransactions(items){
 ...
 items.forEach(bank=>{
  ...
  if(matching.length===1){const receipt=matching[0];Object.assign(receipt,{bankDate:bank.date,valueDate:bank.valueDate||bank.date,method:bank.method||receipt.method,status:'confirmed',source:'Kontoauszug + Kassenbon',note:'Vom Konto abgebucht: '+new Date(bank.date).toLocaleDateString('de-DE')});delete receipt.matchId;merged++;return}
...
function importReceiptTransaction(receipt){
 ...
 if(matching.length===1){
  ...
  Object.assign(bank,receipt,{id:bank.id,date:receipt.date,bankDate,valueDate:bank.valueDate||bankDate,method:bank.method||receipt.method,status:'confirmed',source:'Kontoauszug + Kassenbon',note:'Vom Konto abgebucht: '+new Date(bank.date).toLocaleDateString('de-DE')});delete bank.matchId;
