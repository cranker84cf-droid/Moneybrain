(function(){
 ...
 function rareMonthlyProvision(fixed){return num(fixed.all().filter(item=>item.active&&rhythmMonths(item)>1).reduce((sum,item)=>sum+fixed.planningAmount(item)/rhythmMonths(item),0))}
 function forecast(transactions=[],start=new Date(),days=defaultHorizon){
  ...
  const safety=reserve(),rareMonthly=rareMonthlyProvision(fixed),rareDaily=num(rareMonthly/averageMonthDays),constraints=[];let balance=num(accounts.available),minimum=balance;
  ...
  return {available:num(accounts.available),protectedAmount:num(expenseTotal+rareProvision),expenseTotal,rareDaily,rareMonthly,rareProvision,safety,free,daily,budgetDays:horizonDays,horizonDays,events,minimum,covered:minimum>=safety,nextIncome:nextIncome||null};
 }
