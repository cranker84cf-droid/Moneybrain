(function(){
 ...
 let session=null,uploadTimer=null,accountUploadTimer=null,reconciliationUploadTimer=null,movementUploadTimer=null,categoryUploadTimer=null,syncing=false,accountsSyncing=false,movementsSyncing=false;
 ...
 async function cloudSplits(){return request('/rest/v1/money_transaction_splits?select=id,transaction_id,split_kind,amount,label,account_id,transfer_id,created_at&order=created_at.asc',{headers:headers(session.access_token)})}
 async function cloudCategories(){return request('/rest/v1/money_categories?select=id,name,subcategories,is_builtin,sort_order,created_at,updated_at&order=sort_order.asc',{headers:headers(session.access_token)})}
 async function cloudMerchantRules(){return request('/rest/v1/money_merchant_rules?select=id,merchant_key,category_id,subcategory,created_at,updated_at',{headers:headers(session.access_token)})}
 ...
 async function uploadCategories(){if(!session||!window.MoneybrainCategories)return;const rows=window.MoneybrainCategories.exportRows(),categoryRows=rows.categories.map(item=>({...item,user_id:session.user.id})),ruleRows=rows.rules.map(item=>({...item,user_id:session.user.id}));if(categoryRows.length)await request('/rest/v1/money_categories?on_conflict=user_id,id',{method:'POST',headers:headers(session.access_token,{'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=minimal'}),body:JSON.stringify(categoryRows)});if(ruleRows.length)await request('/rest/v1/money_merchant_rules?on_conflict=id',{method:'POST',headers:headers(session.access_token,{'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=minimal'}),body:JSON.stringify(ruleRows)});setCloudStatus('Gesichert')}
 function queueCategories(){if(!configured()||!session)return;clearTimeout(categoryUploadTimer);categoryUploadTimer=setTimeout(()=>uploadCategories().catch(error=>{setCloudStatus('Nicht synchronisiert');console.error(error)}),500)}
 ...
 async function synchronizeAccounts(){if(!window.MoneybrainAccounts)return;const [rows,reconciliationRows,transferRows,splitRows,categoryRows,ruleRows]=await Promise.all([cloudAccounts(),cloudReconciliations(),cloudTransfers(),cloudSplits(),cloudCategories(),cloudMerchantRules()]);if(Array.isArray(rows)&&rows.length)window.MoneybrainAccounts.applyCloudRows(rows);else await uploadAccounts(window.MoneybrainAccounts.all());window.MoneybrainAccounts.applyCloudReconciliations(reconciliationRows);if(window.MoneybrainMovements){if((!transferRows?.length&&!splitRows?.length)&&(window.MoneybrainMovements.allTransfers().length||window.MoneybrainMovements.allSplits().length))await uploadMovements({transfers:window.MoneybrainMovements.allTransfers(),splits:window.MoneybrainMovements.allSplits()});else window.MoneybrainMovements.applyCloud(transferRows,splitRows)}if(window.MoneybrainCategories){if(!categoryRows?.length)await uploadCategories();else window.MoneybrainCategories.applyCloud(categoryRows,ruleRows)}}
 ...
 async function signOut(){
  ...
  clearTimeout(uploadTimer);clearTimeout(accountUploadTimer);clearTimeout(reconciliationUploadTimer);clearTimeout(movementUploadTimer);clearTimeout(categoryUploadTimer);storeSession(null);lockApp();
 ...
 window.MoneybrainCloud={init,queueUpload,queueAccounts,queueReconciliation,queueMovements,queueCategories,replaceTransactionSplits,deleteAccount,signOut,isConfigured:configured,isSignedIn:()=>Boolean(session)};
})();
