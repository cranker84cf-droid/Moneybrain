(function(){
 ...
 function balanceAt(account,transactions=[],at=nowIso()){
  ...
  for(const transaction of transactions){const time=new Date(transaction.date).getTime();if(transaction.amountUncertain||transaction.linkedTransferId||time<=baseTime||time>cutoff)continue;const parts=window.MoneybrainMovements?.splitsFor(transaction.id)||[];if(parts.length){result+=parts.filter(part=>part.kind!=='transfer'&&(part.accountId||transaction.accountId)===account.id&&!(transaction.bankFundingPending&&part.accountId===transaction.pendingFundingAccountId)).reduce((sum,part)=>sum+(part.kind==='income'?number(part.amount):-number(part.amount)),0);continue}if(transaction.accountId===account.id)result+=transaction.type==='income'?number(transaction.amount):-number(transaction.amount)}
  ...
  return account.systemKey==='paypal'||account.systemKey==='wallet'||account.type==='paypal'||account.type==='cash'?Math.max(0,result):result;
 }
 ...
 function assignSourceTransactions(transactions=[]){const giro=accounts.find(account=>account.systemKey==='giro')||accounts.find(account=>account.type==='bank'&&/^girokonto$/i.test(account.name)),paypal=accounts.find(account=>account.systemKey==='paypal')||accounts.find(account=>account.type==='paypal');let changed=0;for(const transaction of transactions){if(transaction.accountId)continue;const source=String(transaction.source||'');const account=/kontoauszug/i.test(source)?giro:/paypal-nachweis/i.test(source)?paypal:null;if(!account)continue;transaction.accountId=account.id;transaction.method=account.name;changed++}return changed}
 const assignStatementTransactions=assignSourceTransactions;
 ...
 function reconcile(accountId,actualBalance,reconciledAt,transactions=[],note=''){const account=accounts.find(item=>item.id===accountId);if(!account)throw new Error('Geldtopf nicht gefunden');const calculated=balanceAt(account,transactions,reconciledAt),floor=account.systemKey==='paypal'||account.systemKey==='wallet'||account.type==='paypal'||account.type==='cash',actual=floor?Math.max(0,number(actualBalance)):number(actualBalance),item=normalizeReconciliation({id:crypto.randomUUID(),accountId,calculatedBalance:calculated,actualBalance:actual,difference:Math.round((actual-calculated+Number.EPSILON)*100)/100,reconciledAt,note,createdAt:nowIso()});reconciliations.push(item);persistReconciliations(item);return item}
