(function(){
 ...
 function assignStatementTransactions(transactions=[]){const giro=accounts.find(account=>account.systemKey==='giro')||accounts.find(account=>account.type==='bank'&&/^girokonto$/i.test(account.name));if(!giro)return 0;let changed=0;for(const transaction of transactions){if(transaction.accountId||!/kontoauszug/i.test(String(transaction.source||'')))continue;transaction.accountId=giro.id;transaction.method=giro.name;changed++}return changed}
