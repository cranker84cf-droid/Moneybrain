(function(){
 ...
 function balanceAt(account,transactions=[],at=nowIso()){
  ...
  for(const transaction of transactions){const time=new Date(transaction.date).getTime();if(transaction.amountUncertain||transaction.linkedTransferId||time<=baseTime||time>cutoff)continue;const parts=window.MoneybrainMovements?.splitsFor(transaction.id)||[];if(parts.length){result+=parts.filter(part=>part.kind!=='transfer'&&(part.accountId||transaction.accountId)===account.id).reduce((sum,part)=>sum+(part.kind==='income'?number(part.amount):-number(part.amount)),0);continue}if(transaction.accountId===account.id)result+=transaction.type==='income'?number(transaction.amount):-number(transaction.amount)}
