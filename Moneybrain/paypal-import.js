...
function parsePayPalActivityText(text,filename='PayPal'){
 ...
 for(let index=0;index<lines.length;index++){
  ...
  const orderNote=paypalOrderSummary(lines);
  transactions.push({id:crypto.randomUUID(),name:paypalCleanMerchant(name),amount,type,date:date.toISOString(),method:'PayPal',status:foreign?'review':'confirmed',source:'PayPal-Nachweis',note:(orderNote||(detail?'PayPal: '+detail:'Aus PayPal-Aktivitäten erkannt'))+(foreign?' · Fremdwährung: Betrag bitte in Euro prüfen':''),amountUncertain:foreign,receipt:{filename,recognizedAt:new Date().toISOString()}});
 }
...
function paypalFundingSources(text,total){const bank=text.match(/(?:deutsche\s+bank(?:\s+ag)?|bankkonto)[\s\S]{0,80}?(\d{1,3}(?:[.\s]\d{3})*[,.]\d{2})\s*€/i),paypal=text.match(/paypal-?guthaben[\s\S]{0,50}?(\d{1,3}(?:[.\s]\d{3})*[,.]\d{2})\s*€/i);if(!bank||!paypal)return [];const bankAmount=paypalSignedMoney(bank[1]),paypalAmount=paypalSignedMoney(paypal[1]);return Math.round((bankAmount+paypalAmount)*100)===Math.round(Number(total)*100)?[{accountHint:'bank',amount:bankAmount,label:'Bankkonto'},{accountHint:'paypal',amount:paypalAmount,label:'PayPal-Guthaben'}]:[]}
