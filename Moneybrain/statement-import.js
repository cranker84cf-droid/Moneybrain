...
function statementBalances(text,year,transactions=[]){const matches=[...String(text).matchAll(/(?:Kontostand|Neuer\s+Saldo|Saldo)(?:\s+(?:am|zum|per))?\s*(?:(\d{2})\.(\d{2})\.(20\d{2}))?[^\n]{0,50}?([+-]?)\s*([\d.]+,\d{2})\s*(?:EUR|€)/gi)],read=match=>{const amount=Number(match[5].replace(/\./g,'').replace(',','.'))*(match[4]==='-'?-1:1),balanceYear=Number(match[3]||year);return {amount:Math.round(amount*100)/100,date:match[1]?new Date(balanceYear,Number(match[2])-1,Number(match[1]),23,59,59).toISOString():null}};if(!matches.length)return {balance:null,balanceDate:null,openingBalance:null,openingBalanceDate:null};const opening=read(matches[0]),current=read(matches.at(-1)),movement=transactions.reduce((sum,item)=>sum+(item.type==='income'?Number(item.amount):-Number(item.amount)),0),expected=Math.round((opening.amount+movement+Number.EPSILON)*100)/100;return {balance:current.amount,balanceDate:current.date,openingBalance:opening.amount,openingBalanceDate:opening.date,statementMovement:Math.round(movement*100)/100,statementExpectedBalance:expected,statementBalanceDifference:Math.round((current.amount-expected+Number.EPSILON)*100)/100}}
