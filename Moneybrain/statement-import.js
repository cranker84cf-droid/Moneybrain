window.parseDeutscheBankStatement=async function(file){
 ...
 return {transactions,pages:pdf.numPages,year,income:sum(transactions,'income'),expense:sum(transactions,'expense'),cashMovements,cashExcluded:Math.round(cashExcluded*100)/100,...statementBalances(full,year,transactions)};
};
function statementBalances(text,year,transactions=[]){const source=String(text),matches=[...source.matchAll(/(?:Kontostand|Neuer\s+Saldo|Saldo)(?:\s+(?:am|zum|per))?\s*(?:(\d{2})\.(\d{2})\.(20\d{2}))?[^\n]{0,50}?([+-]?)\s*([\d.]+,\d{2})\s*(?:EUR|€)/gi)],printed=source.match(/Erstellt\s*am\s*(\d{2})\.(\d{2})\.(20\d{2})\s*um\s*(\d{1,2}):(\d{2})\s*Uhr/i),read=(match,isCurrent=false)=>{const amount=Number(match[5].replace(/\./g,'').replace(',','.'))*(match[4]==='-'?-1:1),balanceYear=Number(match[3]||year),usePrint=isCurrent&&printed&&match[1]===printed[1]&&match[2]===printed[2]&&String(balanceYear)===printed[3];return {amount:Math.round(amount*100)/100,date:match[1]?new Date(balanceYear,Number(match[2])-1,Number(match[1]),usePrint?Number(printed[4]):0,usePrint?Number(printed[5]):0).toISOString():null}};if(!matches.length)return {balance:null,balanceDate:null,openingBalance:null,openingBalanceDate:null};const opening=read(matches[0]),current=read(matches.at(-1),true),movement=transactions.reduce((sum,item)=>sum+(item.type==='income'?Number(item.amount):-Number(item.amount)),0),expected=Math.round((opening.amount+movement+Number.EPSILON)*100)/100;return {balance:current.amount,balanceDate:current.date,openingBalance:opening.amount,openingBalanceDate:opening.date,statementMovement:Math.round(movement*100)/100,statementExpectedBalance:expected,statementBalanceDifference:Math.round((current.amount-expected+Number.EPSILON)*100)/100}}
...
function parseBookedTransactionsExport(lines,pages,full=''){
 ...
 if(!transactions.length)throw new Error('Keine Buchungszeilen erkannt');
 const year=transactions.length?new Date(transactions[0].date).getFullYear():new Date().getFullYear();return {transactions,pages,year,income:sum(transactions,'income'),expense:sum(transactions,'expense'),cashMovements,cashExcluded:Math.round(cashExcluded*100)/100,...statementBalances(full,year,transactions)};
}
function bookedExportParty(details,bookingKind){
 ...
 if(/Adyen/i.test(beneficiary)&&purpose){const direct=purpose.split(/\/\//)[0].replace(/^Verwendungszweck\s*/i,'');return normalizeParty(direct,bookingKind,purpose)}
 const cardPurpose=/Kartenzahlung/i.test(bookingKind)&&purpose?purpose.split(/\/\//)[0]:'';
 return normalizeParty(cardPurpose||beneficiary||purpose||bookingKind,bookingKind,purpose);
}
